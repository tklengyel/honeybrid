...
{
	struct iphdr* fwd = g_memdup(pkt->packet.ip, ntohs(pkt->packet.ip->tot_len));
...
{
    status_t res;
    struct tcp_packet* rst = g_malloc0(
...
status_t test_expected(struct conn_struct* conn, struct pkt_struct* pkt)
{
    ...
    if (pkt->packet.ip->protocol != conn->expected_data.ip_proto)
    {
        ...
        conn->replay_problem = conn->replay_problem | REPLAY_UNEXPECTED_PROTOCOL;
    }
...
status_t tcp_checksum(struct tcp_packet* pkt) {
	...
	struct tcp_chk_packet *chk_p = alloca(sizeof(struct tcp_chk_packet) + padd);
