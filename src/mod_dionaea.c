...
void InfoDest(void *a) {
    printdbg("%s Destroying element in dionaea connection tree!\n",H(0));
...
void handleDionaeaEvent(struct dionaeaEvent *dionaeaEvent) {
    if(dionaeaEvent->dionaea && dionaeaEvent->incident != NULL) {
        ...
        if (reference_exists == FALSE && dionaeaEvent->start == 1) {
            printdbg("%s Creating new node in reference GTree with reference key %u!\n", H(0),dionaeaEvent->reference);
            ...
            *reference_key=dionaeaEvent->reference;
            ...
            printdbg("%s Double start of the same dionaea reference?!\n",H(0));
        }
        ...
        if(connection_exists == FALSE) {
            printdbg("%s Inserting new connection to dionaea conn tree!\n",H(0));
...
void xmpp_listener() {
    ...
    while(1) {
        ...
        if (doc == NULL) {
            printdbg("%s Failed to parse XML from FIFO queue!\n",H(0));
        } else {
...
void xmpp_backup() {
    ...
    while(1) {
        ...
        printdbg("%s Removed %i elements from dionaea conn tree!\n", H(0),counter);
        g_list_free(cleanup);
        g_rw_lock_writer_unlock (&dionaea_connection_lock);
        g_usleep(60000000);
    }
...
int init_mod_dionaea()
{
    ...
    if(NULL != (xmpp_db=(gchar *)g_hash_table_lookup(config,"xmpp_db"))) {
        ...
        if(g_key_file_load_from_file(xmpp_backup_file, xmpp_db,0,NULL)) {
            printdbg("%s Dionaea XMPP backup loaded from file %s\n",H(0),xmpp_db);
        }
    } else {
    ...
    printdbg("%s: Dionaea XMPP listener thread started\n", H(0));
    ...
    printdbg("%s: Dionaea XMPP backup thread started\n", H(0));
...
mod_result_t mod_dionaea(struct mod_args *args)
{
    ...
    if(xmpp_fifo==NULL) {
        ...
        printdbg("%s Dionaea module is not configured properly, can't decide!!\n", H(args->pkt->conn->id));
        args->node->result = -1;
        return result;
    }
    ...
    if ( NULL == (mode = (gchar *)g_hash_table_lookup(args->node->arg, "mode"))) {
        /*! We can't decide */
        args->node->result = -1;
        printdbg("%s mandatory argument 'mode' undefined (front/back)!\n", H(args->pkt->conn->id));
        return;
    }
    ...
    if ( NULL != (info = g_key_file_get_string_list(
        ...
        printdbg("%s XMPP Backup info found: %s\n", H(args->pkt->conn->id),info[0]);
