...
/*! \file mod_vmi_legacy.c
 * \brief Module to interact with VMI-Honeymon version 1 (Perl)
 *
 * This module is called by a boolean decision tree
 *
...
void free_vmi_msg(gpointer data) {
    ...
    if (tofree->vmname != NULL) free(tofree->vmname);
    if (tofree->command != NULL) free(tofree->command);
    free(tofree);
}
...
void check_vm_state(gchar *vmname) {
    ...
    if (now - IP_last_seen >= 60 || now - vmi_session_start >= 600) conn_expired =
            1;
    else conn_expired = 0;
    ...
    if (!g_queue_is_empty(vmi_send)) {
    ...
    } else if (g_queue_is_empty(vmi_send) && !g_queue_is_empty(vmi_recv)) {
        ...
        if (msg->result == 1 || msg->result == 3) {
            ...
            free(vmi_user);
            vmi_user = NULL;
        } else if (msg->result == 0 && conn_expired) {
        ...
        if (conn_expired) {
            // do a final check
            g_printerr(
                    "%s Sending check request to VMI because connection expired!\n",
                    H(22));
            struct vmi_msg *command = (struct vmi_msg *) malloc(
                    sizeof(struct vmi_msg));
...
void vmi_com() {
    while (1) {
            ...
            if ((vmi_sock = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP)) < 0
                ...
                if (send(vmi_sock, send_str, strlen(send_str), 0)
                    ...
                    if (recv(vmi_sock, vmi_buffer, 100, 0) <= 0) {
                        ...
                        struct vmi_msg *msg = (struct vmi_msg *) malloc(
                                sizeof(struct vmi_msg));
        ...
        g_rw_lock_writer_unlock(&vmi_lock);
...
int init_mod_vmi_legacy() {
    ...
    if (NULL
            == (vmi_server_ip = (gchar *) g_hash_table_lookup(config,
                    "vmi_legacy_server_ip"))) {
        // Not defined so skipping init
        return 0;
    }

    if (NULL
            == (vmi_server_port = (int *) g_hash_table_lookup(config,
                    "vmi_legacy_server_port"))) {
    ...
    vmi_addr.sin_port = htons(*vmi_server_port); /* Server port */
    ...
    if ((vmi_com_thread = g_thread_new("vmi_com", (void *) vmi_com, NULL)) == NULL) errx(1,
            "%s: Unable to start the VMI communication thread", __func__);
    else g_printerr("%s: VMI communication thread started\n", H(22));
...
mod_result_t mod_vmi_legacy(struct mod_args *args) {
    ...
    mod_result_t res = REJECT;

    GTimeVal t;
    g_get_current_time(&t);
    gint now = (t.tv_sec);

    char *key_src = args->pkt->src;
    char *key_dst = args->pkt->dst;
    ...
    if (NULL
            == (vmname = (gchar *) g_hash_table_lookup(args->node->config,
                    "vmname"))) {
        /*! We can't decide */
    ...
    if (g_rw_lock_writer_trylock(&vmi_lock)) {

        g_printerr("%s Got the lock for VMI\n", H(22));

        if (vmi_user != NULL) check_vm_state(vmname);
        ...
        if (strcmp(mode, "back") == 0) {
            if (vmi_user == NULL) {
                if (args->pkt->packet.ip->protocol == 6) {
                    ...
                    g_printerr(
                            "%s Vm is free, but I don't like this connection\n",
                            H(22));
                }
            } else if (strcmp(key_src, vmi_user) != 0) {
                // vm is used, decline
                ...
                IP_last_seen = now;
                g_printerr(
                        "%s Vm is used by this IP: %s and you are: %s, accepted!\n",
                        H(22), vmi_user, key_src);
            }
        }
        ...
        if (strcmp(mode, "control") == 0) {
            if (vmi_user == NULL) {
                if (args->pkt->origin == HIH) {
                    ...
                    g_printerr(
                            "%s No forwarded session and the VM is generating outbound traffic\n",
                            H(22));
