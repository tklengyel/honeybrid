...
GString * create_pcap_filename(int mode)
{
	 ...
	 */
	g_string_append_printf(name,"%s",(gchar *)g_hash_table_lookup(config,"conn_record"));
	...
	g_string_append_printf(name,"%d%d%d-%d-%d-%d",(actualtime->tm_year +1900), (actualtime->tm_mon +1), actualtime->tm_mday, actualtime->tm_hour, actualtime->tm_min, actualtime->tm_sec);
...
///		L(32,"INIT_PCAP_CONTEXT",NULL,5);
	...
	*/
	if (NULL == (pcap_output_current = pcap_dump_open(pcap_main_desc, output_file_name->str)))
	{
...
///	L(34,"INIT_PCAP_CONTEXT",output_file_name->str, 5);
	 ...
	 */
	if (NULL == (pcap_output_redirected = pcap_dump_open(pcap_main_desc, redirected_output_file_name->str)))
	{
...
///		L(34,"RECORD_PKT",file_name->str,5);
	...
	else
	{
		...
		*/
		pcap_dump((void *)DumpDescriptor, &phdr, (const u_char *)netfilter_packet);
	}
